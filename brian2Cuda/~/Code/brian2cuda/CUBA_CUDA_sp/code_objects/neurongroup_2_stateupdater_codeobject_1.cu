...
__global__ void
kernel_neurongroup_2_stateupdater_codeobject_1(
    int _N,
    int THREADS_PER_BLOCK,
    ///// KERNEL_PARAMETERS /////
    double* _ptr_array_neurongroup_2_s_AMPA,
	double* _ptr_array_neurongroup_2_s_GABA,
    ...
    )
{
    ...
    const double _lio_2 = 1.0f*(- dt)/0.01;
    const double _lio_3 = 1.0f*(- dt)/0.005;
    const double _lio_4 = 1.0f*dt/1.0699999999999999e-10;
    const double _lio_5 = - (-0.068);
    const double _lio_6 = - (-0.07);
    const double _lio_7 = - 0.0;
    const double _lio_8 = - (-0.075);
...

    {
        ...
        double s_AMPA = _ptr_array_neurongroup_2_s_AMPA[_idx];
        double s_AHP_PKJ = _ptr_array_neurongroup_2_s_AHP_PKJ[_idx];
        double s_GABA = _ptr_array_neurongroup_2_s_GABA[_idx];
        const double _s_AMPA = (_lio_1 * s_AMPA) + s_AMPA;
        const double _s_GABA = (_lio_2 * s_GABA) + s_GABA;
        const double _s_AHP_PKJ = (_lio_3 * s_AHP_PKJ) + s_AHP_PKJ;
        const double _V = V + (_lio_4 * (2.5e-10 - ((((2.32e-09 * (_lio_5 + V)) + (1.0000000000000002e-10 * (s_AHP_PKJ * (_lio_6 + V)))) + (7e-10 * (s_AMPA * (_lio_7 + V)))) + (1e-09 * (s_GABA * (_lio_8 + V))))));
        s_AMPA = _s_AMPA;
        s_GABA = _s_GABA;
        s_AHP_PKJ = _s_AHP_PKJ;
        V = _V;
        _ptr_array_neurongroup_2_V[_idx] = V;
        _ptr_array_neurongroup_2_s_AMPA[_idx] = s_AMPA;
        _ptr_array_neurongroup_2_s_AHP_PKJ[_idx] = s_AHP_PKJ;
        _ptr_array_neurongroup_2_s_GABA[_idx] = s_GABA;
...
void _run_neurongroup_2_stateupdater_codeobject_1()
{
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    kernel_neurongroup_2_stateupdater_codeobject_1, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    kernel_neurongroup_2_stateupdater_codeobject_1, num_threads, 0)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, kernel_neurongroup_2_stateupdater_codeobject_1)
                );
        ...
        {
            printf("INFO kernel_neurongroup_2_stateupdater_codeobject_1\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
    ...
    kernel_neurongroup_2_stateupdater_codeobject_1<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
            dev_array_neurongroup_2_s_AMPA,
			dev_array_neurongroup_2_s_GABA,
			dev_array_neurongroup_2_s_AHP_PKJ,
			dev_array_neurongroup_2_V,
			_array_defaultclock_dt[0]
        );

    CUDA_CHECK_ERROR("kernel_neurongroup_2_stateupdater_codeobject_1");
    ...
    neurongroup_2_stateupdater_codeobject_1_profiling_info += _run_time;
}
