...
__global__ void
kernel_statemonitor_1_codeobject(
    int _num_indices,
    int32_t* indices,
...
if (_num__array_statemonitor_1__indices > 1024)
{
    printf("ERROR in statemonitor_1: Too many neurons recorded. Due to a bug (brian-team/brian2cuda#50), "
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, kernel_statemonitor_1_codeobject)
                );
...
{
    kernel_statemonitor_1_codeobject<<<1, _num__array_statemonitor_1__indices>>>(
    ...
    CUDA_CHECK_ERROR("kernel_statemonitor_1_codeobject");
}
