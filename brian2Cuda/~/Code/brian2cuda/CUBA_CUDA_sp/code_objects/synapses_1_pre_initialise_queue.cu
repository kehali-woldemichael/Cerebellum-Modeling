...
void _run_synapses_1_pre_initialise_queue()
{
    ...
    int64_t syn_N_check = _array_synapses_1_N[0];
    if (syn_N_check == 0){
        return;
    }
    ...
    _dynamic_array_synapses_1_delay = dev_dynamic_array_synapses_1_delay;
    ...
    for(int i = 0; i < num_pre_post_blocks; i++)
    {
        ...
        if (!scalar_delay)
        {
            ...
            for (int bundle_idx = 0; bundle_idx < num_unique_elements; bundle_idx++)
            {
                ...
                if (num_synapses > synapses_1_pre_max_bundle_size)
                    synapses_1_pre_max_bundle_size = num_synapses;
    ...
    else  // not scalar_delay
    {
        ...
        synapses_1_pre_mean_bundle_size = sum_bundle_sizes / num_bundle_ids;
    ...
    if (scalar_delay)
    {
        int num_spikespaces = dev_array_poissongroup_1__spikespace.size();
        if (num_queues > num_spikespaces)
        {
            for (int i = num_spikespaces; i < num_queues; i++)
            {
                ...
                cudaError_t status = cudaMalloc((void**)&new_eventspace,
                        sizeof(int32_t)*_num__array_poissongroup_1__spikespace);
                if (status != cudaSuccess)
                {
                    printf("ERROR while allocating momory for dev_array_poissongroup_1__spikespace[%i] on device: %s %s %d\n",
                            i, cudaGetErrorString(status), __FILE__, __LINE__);
                    exit(status);
                }
                dev_array_poissongroup_1__spikespace.push_back(new_eventspace);
            }
        }
    }
