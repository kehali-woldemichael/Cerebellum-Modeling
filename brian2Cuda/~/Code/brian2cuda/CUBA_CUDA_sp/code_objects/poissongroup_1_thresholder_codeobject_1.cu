#include "objects.h"
#include "code_objects/poissongroup_1_thresholder_codeobject_1.h"
...
namespace {
    ...
    double host_randn(const int _vectorisation_idx)
    {
        ...
        __global__ void
        _reset_poissongroup_1_thresholder_codeobject_1(
            int32_t* eventspace
            )
        {
    ...
    #define _rand(vectorisation_idx) (_ptr_array_poissongroup_1_thresholder_codeobject_1_rand[vectorisation_idx])
    template <typename T>
    __host__ __device__
...
__global__ void
kernel_poissongroup_1_thresholder_codeobject_1(
    int _N,
    int THREADS_PER_BLOCK,
    ///// KERNEL_PARAMETERS /////
    int32_t* _ptr_array_poissongroup_1__spikespace,
	const double _value_array_defaultclock_t,
	const double _value_array_defaultclock_dt,
	double* _ptr_array_poissongroup_1_thresholder_codeobject_1_rand
    )
{
    using namespace brian;
    ...
    const double _lio_1 = (2.25 + (0.75 * _brian_cos((2.0 * ((3.141592653589793 * t) * 0.5)) + 3.141592653589793))) * 1.0;
    ...
    if (_cond)
    {
        int32_t spike_index = atomicAdd(&_ptr_array_poissongroup_1__spikespace[_N], 1);
        _ptr_array_poissongroup_1__spikespace[spike_index] = _idx;
    }
    }
}

void _run_poissongroup_1_thresholder_codeobject_1()
{
    using namespace brian;
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    kernel_poissongroup_1_thresholder_codeobject_1, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    kernel_poissongroup_1_thresholder_codeobject_1, num_threads, 0)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, kernel_poissongroup_1_thresholder_codeobject_1)
                );
        ...
        {
            printf("INFO kernel_poissongroup_1_thresholder_codeobject_1\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
        ...
        _reset_poissongroup_1_thresholder_codeobject_1<<<num_blocks, num_threads>>>(
                dev_array_poissongroup_1__spikespace[current_idx_array_poissongroup_1__spikespace]
            );

        CUDA_CHECK_ERROR("_reset_poissongroup_1_thresholder_codeobject_1");

    kernel_poissongroup_1_thresholder_codeobject_1<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
            dev_array_poissongroup_1__spikespace[current_idx_array_poissongroup_1__spikespace],
			...
			dev_poissongroup_1_thresholder_codeobject_1_rand
        );

    CUDA_CHECK_ERROR("kernel_poissongroup_1_thresholder_codeobject_1");
    ...
    poissongroup_1_thresholder_codeobject_1_profiling_info += _run_time;
}
