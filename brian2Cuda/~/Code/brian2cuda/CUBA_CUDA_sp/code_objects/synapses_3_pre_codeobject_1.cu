...
__global__ void
kernel_synapses_3_pre_codeobject_1(
    int _N,
    int bid_offset,
    int timestep,
    ...
    )
{
    ...
    {  // _idx is defined in outer and inner scope (for `scalar_code`)
        if (synapses_3_pre.no_or_const_delay_mode)
        {
            ...
            if (timestep >= synapses_3_pre.queue->num_queues - 1)
            {
                ...
                int post_block_idx = bid % num_parallel_blocks;
                {
                    ...
                    if(spikes_start <= spiking_neuron && spiking_neuron < spikes_stop)
                    {
                        ...
                        for(int j = tid; j < num_synapses; j+=THREADS_PER_BLOCK)
                        {
                            ...
                            //  Abstract code:  s_GABA_1 += 1
                            //  Abstract code:  s_GABA_2 += 1
                            const int32_t _postsynaptic_idx = _ptr_array_synapses_3__synaptic_post[_idx];
                            _brian_atomicAdd(&_ptr_array_neurongroup_s_GABA_1[_postsynaptic_idx], (double)(1));
                            _brian_atomicAdd(&_ptr_array_neurongroup_s_GABA_2[_postsynaptic_idx], (double)(1));
...
void _run_synapses_3_pre_codeobject_1()
{
    ...
    ///// HOST_CONSTANTS ///////////
    int32_t* const _array_synapses_3__synaptic_post = thrust::raw_pointer_cast(&dev_dynamic_array_synapses_3__synaptic_post[0]);
		const int _num_postsynaptic_idx = dev_dynamic_array_synapses_3__synaptic_post.size();
		int32_t* const _array_synapses_3__synaptic_pre = thrust::raw_pointer_cast(&dev_dynamic_array_synapses_3__synaptic_pre[0]);
...
num_threads_per_bundle = synapses_3_pre_max_bundle_size;
num_loops = 1;
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    kernel_synapses_3_pre_codeobject_1, num_threads, 0)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, kernel_synapses_3_pre_codeobject_1)
                );
...
else if (synapses_3_pre_max_size <= 0)
{
    printf("INFO there are no synapses in the synapses_3_pre pathway. Skipping synapses_push and synapses kernels.\n");
        ...
        {
            printf("INFO kernel_synapses_3_pre_codeobject_1\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
if (synapses_3_pre_max_size > 0)
{
    ...
    if (num_blocks != 0) {
        ...
        {
            kernel_synapses_3_pre_codeobject_1<<<num_blocks, num_threads>>>(
                _N,
                bid_offset,
                ...
                num_spiking_neurons,
                ///// HOST_PARAMETERS /////
                _array_synapses_3__synaptic_post,
			_num_postsynaptic_idx,
			_array_synapses_3__synaptic_pre,
			_num_synaptic_pre,
    ...
    CUDA_CHECK_ERROR("kernel_synapses_3_pre_codeobject_1");
}
    ...
    synapses_3_pre_codeobject_1_profiling_info += _run_time;
}
