...
__global__ void
kernel_synapses_1_post_codeobject_1(
    int _N,
    int bid_offset,
    int timestep,
...
num_loops = 1;
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    kernel_synapses_1_post_codeobject_1, num_threads, 0)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, kernel_synapses_1_post_codeobject_1)
                );
...
else if (synapses_1_post_max_size <= 0)
{
    printf("INFO there are no synapses in the synapses_1_post pathway. Skipping synapses_push and synapses kernels.\n");
        ...
        {
            printf("INFO kernel_synapses_1_post_codeobject_1\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
if (synapses_1_post_max_size > 0)
{
    ...
    {
        if (defaultclock.timestep[0] >= synapses_1_post_delay)
        {
            cudaMemcpy(&num_spiking_neurons,
                    &dev_array_neurongroup_2__spikespace[synapses_1_post_eventspace_idx][_num__array_neurongroup_2__spikespace - 1],
    ...
    if (num_blocks != 0) {
        ...
        {
            kernel_synapses_1_post_codeobject_1<<<num_blocks, num_threads>>>(
                _N,
                bid_offset,
                defaultclock.timestep[0],
                num_threads,
                num_threads_per_bundle,
                dev_array_neurongroup_2__spikespace[synapses_1_post_eventspace_idx],
                num_spiking_neurons,
                ///// HOST_PARAMETERS /////
    ...
    CUDA_CHECK_ERROR("kernel_synapses_1_post_codeobject_1");
}
    ...
    synapses_1_post_codeobject_1_profiling_info += _run_time;
}
