#include "objects.h"
#include "code_objects/statemonitor_3_codeobject_1.h"
...
__global__ void
kernel_statemonitor_3_codeobject_1(
    int _num_indices,
    int32_t* indices,
	...
	double* _ptr_array_statemonitor_3_t,
	const int _numt,
	int32_t* _ptr_array_statemonitor_3_N
    )
{
...
void _run_statemonitor_3_codeobject_1()
{
    using namespace brian;
    ...
    ///// HOST_CONSTANTS ///////////
    const int _num_source_V = 10;
		double* const _array_statemonitor_3_t = thrust::raw_pointer_cast(&dev_dynamic_array_statemonitor_3_t[0]);
		const int _numt = dev_dynamic_array_statemonitor_3_t.size();
		const int _numN = 1;
...
// on the device?
dev_dynamic_array_statemonitor_3_t.push_back(defaultclock.t[0]);
...
{
    _dynamic_array_statemonitor_3_V[i].resize(_numt + num_iterations - current_iteration);
    addresses_monitor__dynamic_array_statemonitor_3_V.push_back(thrust::raw_pointer_cast(&_dynamic_array_statemonitor_3_V[i][0]));
}
// Print a warning when the monitor is not going to work (#50)
if (_num__array_statemonitor_3__indices > 1024)
{
    printf("ERROR in statemonitor_3: Too many neurons recorded. Due to a bug (brian-team/brian2cuda#50), "
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, kernel_statemonitor_3_codeobject_1)
                );
...
if(current_iteration >= num_iterations)
{
    for(int i = 0; i < _num__array_statemonitor_3__indices; i++)
    {
        _dynamic_array_statemonitor_3_V[i].resize(_numt + 1);
        addresses_monitor__dynamic_array_statemonitor_3_V[i] = thrust::raw_pointer_cast(&_dynamic_array_statemonitor_3_V[i][0]);
    }
}

if (_num__array_statemonitor_3__indices > 0)
// TODO we get invalid launch configuration if this is 0, which happens e.g. for StateMonitor(..., variables=[])
{
    kernel_statemonitor_3_codeobject_1<<<1, _num__array_statemonitor_3__indices>>>(
        _num__array_statemonitor_3__indices,
        dev_array_statemonitor_3__indices,
        current_iteration - start_offset,
        thrust::raw_pointer_cast(&addresses_monitor__dynamic_array_statemonitor_3_V[0]),
        ///// HOST_PARAMETERS /////
        dev_array_neurongroup_1_V,
			_array_statemonitor_3_t,
			_numt,
			dev_array_statemonitor_3_N
        );

    CUDA_CHECK_ERROR("kernel_statemonitor_3_codeobject_1");
}
    ...
    statemonitor_3_codeobject_1_profiling_info += _run_time;
}
