...
__global__ void _run_synapses_6_pre_initialise_queue_kernel(
    int _source_N,
    int _num_blocks,
    int _num_threads,
    double _dt,
    ...
    bool new_mode)
{
    ...
    synapses_6_pre.no_or_const_delay_mode = new_mode;
}
...
void _run_synapses_6_pre_initialise_queue()
{
    ...
    int64_t syn_N_check = _array_synapses_6_N[0];
    if (syn_N_check == 0){
        return;
    }
    ...
    _dynamic_array_synapses_6_delay = dev_dynamic_array_synapses_6_delay;
    ...
    for(int i = 0; i < num_pre_post_blocks; i++)
    {
        ...
        if (!scalar_delay)
        {
            ...
            for (int bundle_idx = 0; bundle_idx < num_unique_elements; bundle_idx++)
            {
                ...
                if (num_synapses > synapses_6_pre_max_bundle_size)
                    synapses_6_pre_max_bundle_size = num_synapses;
    ...
    if (scalar_delay)
    {
        ...
        COPY_HOST_ARRAY_TO_DEVICE_SYMBOL(h_num_synapses_by_pre,
                synapses_6_pre_num_synapses_by_pre, num_pre_post_blocks,
                "number of synapses per pre/post block");
        // synapses id
        COPY_HOST_ARRAY_TO_DEVICE_SYMBOL(d_ptr_synapse_ids_by_pre,
                synapses_6_pre_synapse_ids_by_pre, num_pre_post_blocks,
    ...
    else  // not scalar_delay
    {
        ...
        synapses_6_pre_mean_bundle_size = sum_bundle_sizes / num_bundle_ids;
    ...
    cudaFuncGetAttributes(&funcAttrib, _run_synapses_6_pre_initialise_queue_kernel);
    if (num_threads > funcAttrib.maxThreadsPerBlock)
    {
        // use the max num_threads before launch failure
        num_threads = funcAttrib.maxThreadsPerBlock;
        printf("WARNING Not enough ressources available to call "
               "_run_synapses_6_pre_initialise_queue_kernel "
    ...
    {
        printf("INFO _run_synapses_6_pre_initialise_queue_kernel\n"
               "\t%u blocks\n"
               "\t%u threads\n"
    ...
    _run_synapses_6_pre_initialise_queue_kernel<<<num_blocks, num_threads>>>(
        source_N,
        num_parallel_blocks,
        num_threads,
        dt,
