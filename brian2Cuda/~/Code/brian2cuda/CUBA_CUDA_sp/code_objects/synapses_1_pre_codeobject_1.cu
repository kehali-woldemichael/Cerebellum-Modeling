...
__global__ void
kernel_synapses_1_pre_codeobject_1(
    int _N,
    int bid_offset,
    int timestep,
    ...
    ///// KERNEL_PARAMETERS /////
    double* _ptr_array_synapses_1_w_CFPKJ,
	const int _numw_CFPKJ,
                ...
                int post_block_idx = bid % num_parallel_blocks;
                {
                    ...
                    if(spikes_start <= spiking_neuron && spiking_neuron < spikes_stop)
                    {
                        ...
                        for(int j = tid; j < num_synapses; j+=THREADS_PER_BLOCK)
                        {
                            ...
                            //  Abstract code:  s_AMPA += w_CFPKJ
                            const int32_t _postsynaptic_idx = _ptr_array_synapses_1__synaptic_post[_idx];
                            const double w_CFPKJ = _ptr_array_synapses_1_w_CFPKJ[_idx];
                            _brian_atomicAdd(&_ptr_array_neurongroup_2_s_AMPA[_postsynaptic_idx], (double)(w_CFPKJ));
...
void _run_synapses_1_pre_codeobject_1()
{
    ...
    ///// HOST_CONSTANTS ///////////
    double* const _array_synapses_1_w_CFPKJ = thrust::raw_pointer_cast(&dev_dynamic_array_synapses_1_w_CFPKJ[0]);
		const int _numw_CFPKJ = dev_dynamic_array_synapses_1_w_CFPKJ.size();
		const int _nums_AMPA = 10;
...
num_loops = 1;
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    kernel_synapses_1_pre_codeobject_1, num_threads, 0)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, kernel_synapses_1_pre_codeobject_1)
                );
...
else if (synapses_1_pre_max_size <= 0)
{
    printf("INFO there are no synapses in the synapses_1_pre pathway. Skipping synapses_push and synapses kernels.\n");
        ...
        {
            printf("INFO kernel_synapses_1_pre_codeobject_1\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
if (synapses_1_pre_max_size > 0)
{
    ...
    if (synapses_1_pre_scalar_delay)
    {
        if (defaultclock.timestep[0] >= synapses_1_pre_delay)
        {
            cudaMemcpy(&num_spiking_neurons,
                    &dev_array_poissongroup_1__spikespace[synapses_1_pre_eventspace_idx][_num__array_poissongroup_1__spikespace - 1],
    ...
    if (num_blocks != 0) {
        ...
        {
            kernel_synapses_1_pre_codeobject_1<<<num_blocks, num_threads>>>(
                _N,
                bid_offset,
                defaultclock.timestep[0],
                num_threads,
                num_threads_per_bundle,
                dev_array_poissongroup_1__spikespace[synapses_1_pre_eventspace_idx],
                num_spiking_neurons,
                ///// HOST_PARAMETERS /////
                _array_synapses_1_w_CFPKJ,
			_numw_CFPKJ,
    ...
    CUDA_CHECK_ERROR("kernel_synapses_1_pre_codeobject_1");
}
    ...
    synapses_1_pre_codeobject_1_profiling_info += _run_time;
}
