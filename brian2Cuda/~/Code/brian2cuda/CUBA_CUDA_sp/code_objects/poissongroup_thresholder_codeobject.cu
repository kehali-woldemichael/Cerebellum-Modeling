...
namespace {
    ...
    #define _rand(vectorisation_idx) (_ptr_array_poissongroup_thresholder_codeobject_rand[vectorisation_idx])
    template <typename T>
    __host__ __device__
...
kernel_poissongroup_thresholder_codeobject(
	...
	double* _ptr_array_poissongroup_thresholder_codeobject_rand
    )
{
    using namespace brian;
...
void _run_poissongroup_thresholder_codeobject()
{
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    kernel_poissongroup_thresholder_codeobject, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    kernel_poissongroup_thresholder_codeobject, num_threads, 0)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, kernel_poissongroup_thresholder_codeobject)
                );
        ...
        {
            printf("INFO kernel_poissongroup_thresholder_codeobject\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
        ...
        _reset_poissongroup_thresholder_codeobject<<<num_blocks, num_threads>>>(
                dev_array_poissongroup__spikespace[current_idx_array_poissongroup__spikespace]
            );

        CUDA_CHECK_ERROR("_reset_poissongroup_thresholder_codeobject");

    kernel_poissongroup_thresholder_codeobject<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
            dev_array_poissongroup__spikespace[current_idx_array_poissongroup__spikespace],
    ...
    CUDA_CHECK_ERROR("kernel_poissongroup_thresholder_codeobject");
