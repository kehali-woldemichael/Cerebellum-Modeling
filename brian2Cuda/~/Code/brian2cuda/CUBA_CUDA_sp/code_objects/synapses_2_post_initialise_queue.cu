...
__global__ void _run_synapses_2_post_initialise_queue_kernel(
    int _source_N,
    int _num_blocks,
    int _num_threads,
    double _dt,
    ...
    bool new_mode)
{
    ...
    synapses_2_post.queue->prepare(
        ...
        _syn_N,
        num_queues,
        synapses_2_post_num_synapses_by_pre,
        synapses_2_post_num_synapses_by_bundle,
        synapses_2_post_num_unique_delays_by_pre,
        synapses_2_post_unique_delays,
        synapses_2_post_global_bundle_id_start_by_pre,
        synapses_2_post_synapses_offset_by_bundle,
        synapses_2_post_synapse_ids,
        synapses_2_post_synapse_ids_by_pre,
        synapses_2_post_unique_delays_offset_by_pre,
        synapses_2_post_unique_delay_start_idcs);
    synapses_2_post.no_or_const_delay_mode = new_mode;
}
...
void _run_synapses_2_post_initialise_queue()
{
    ...
    int64_t syn_N_check = _array_synapses_2_N[0];
    if (syn_N_check == 0){
        return;
    }
    ...
    _dynamic_array_synapses_2_delay_1 = dev_dynamic_array_synapses_2_delay_1;
    ...
    int max_delay = (int)(_dynamic_array_synapses_2_delay_1[0] / dt + 0.5);
    ...
    for(int syn_id = 0; syn_id < syn_N; syn_id++)  // loop through all synapses
    {
        ...
        int32_t pre_neuron_id = _dynamic_array_synapses_2__synaptic_post[syn_id] - 0;
        int32_t post_neuron_id = _dynamic_array_synapses_2__synaptic_pre[syn_id] - 0;

        int delay = (int)(_dynamic_array_synapses_2_delay_1[syn_id] / dt + 0.5);
        if (delay > max_delay)
            max_delay = delay;
        if (delay < min_delay)
            min_delay = delay;
    ...
    if (scalar_delay)
        synapses_2_post_delay = max_delay;
    ...
    dev_dynamic_array_synapses_2_delay_1.shrink_to_fit();
    CUDA_CHECK_MEMORY();
    ...
    for(int i = 0; i < num_pre_post_blocks; i++)
    {
        ...
        if (num_elements > synapses_2_post_max_size)
            synapses_2_post_max_size = num_elements;
        ...
        if (!scalar_delay)
        {
            ...
            if (num_unique_elements > synapses_2_post_max_num_unique_delays)
                synapses_2_post_max_num_unique_delays = num_unique_elements;
            ...
            for (int bundle_idx = 0; bundle_idx < num_unique_elements; bundle_idx++)
            {
                ...
                if (num_synapses > synapses_2_post_max_bundle_size)
                    synapses_2_post_max_bundle_size = num_synapses;
    ...
    if (scalar_delay)
    {
        ...
        COPY_HOST_ARRAY_TO_DEVICE_SYMBOL(h_num_synapses_by_pre,
                synapses_2_post_num_synapses_by_pre, num_pre_post_blocks,
                "number of synapses per pre/post block");
        // synapses id
        COPY_HOST_ARRAY_TO_DEVICE_SYMBOL(d_ptr_synapse_ids_by_pre,
                synapses_2_post_synapse_ids_by_pre, num_pre_post_blocks,
    ...
    else  // not scalar_delay
    {
        ...
        CUDA_SAFE_CALL(
                cudaMemcpyToSymbol(synapses_2_post_unique_delays,
                                   &d_ptr_unique_delays,
        ...
        synapses_2_post_mean_bundle_size = sum_bundle_sizes / num_bundle_ids;

        // pointer to start of synapse IDs array
        CUDA_SAFE_CALL(
                cudaMemcpyToSymbol(synapses_2_post_synapse_ids, &d_ptr_synapse_ids,
        ...
        COPY_HOST_ARRAY_TO_DEVICE_SYMBOL(
                ...
                synapses_2_post_num_synapses_by_bundle, num_bundle_ids,
                "number of synapses per bundle");

        // synapses offset by bundle
        COPY_HOST_ARRAY_TO_DEVICE_SYMBOL(
                thrust::raw_pointer_cast(&h_synapses_offset_by_bundle[0]),
                synapses_2_post_synapses_offset_by_bundle, num_bundle_ids,
    ...
    std::cout << "INFO: synapse statistics and memory usage for synapses_2_post:\n"
    ...
    cudaFuncGetAttributes(&funcAttrib, _run_synapses_2_post_initialise_queue_kernel);
    if (num_threads > funcAttrib.maxThreadsPerBlock)
    {
        // use the max num_threads before launch failure
        num_threads = funcAttrib.maxThreadsPerBlock;
        printf("WARNING Not enough ressources available to call "
               "_run_synapses_2_post_initialise_queue_kernel "
    ...
    {
        printf("INFO _run_synapses_2_post_initialise_queue_kernel\n"
               "\t%u blocks\n"
               "\t%u threads\n"
    ...
    _run_synapses_2_post_initialise_queue_kernel<<<num_blocks, num_threads>>>(
        source_N,
        num_parallel_blocks,
        num_threads,
        dt,
    ...
    synapses_2_post_scalar_delay = scalar_delay;

    cudaError_t status = cudaGetLastError();
    if (status != cudaSuccess)
    {
        printf("ERROR initialising synapses_2_post in %s:%d %s\n",
    ...
    std::cout << "INFO: synapses_2_post initialisation took " <<  time_passed << "s";
