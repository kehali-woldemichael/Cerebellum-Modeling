...
__global__ void _run_synapses_4_pre_push_spikes_advance_kernel()
{
...
__global__ void
_run_synapses_4_pre_push_spikes_push_kernel(
...
void _run_synapses_4_pre_push_spikes()
{
    ...
    if (synapses_4_pre_scalar_delay)
    {
        ...
        synapses_4_pre_eventspace_idx = (current_idx_array_neurongroup__spikespace - synapses_4_pre_delay + num_eventspaces) % num_eventspaces;
    ...
    else if (synapses_4_pre_max_size > 0)
    {
        ...
        _run_synapses_4_pre_push_spikes_advance_kernel<<<1, num_parallel_blocks>>>();

        CUDA_CHECK_ERROR("_run_synapses_4_pre_push_spikes_advance_kernel");
        ...
        if (first_run)
        {
            ...
            CUDA_SAFE_CALL(
                    cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                        _run_synapses_4_pre_push_spikes_push_kernel, num_threads,
            ...
            CUDA_SAFE_CALL(
                    cudaFuncGetAttributes(&funcAttrib, _run_synapses_4_pre_push_spikes_push_kernel)
                    );
            ...
            {
                printf("INFO _run_synapses_4_pre_push_spikes_push_kernel\n"
        ...
        if (num_spiking_neurons > 0)
        {
            ...
            _run_synapses_4_pre_push_spikes_push_kernel<<<num_blocks, num_threads, needed_shared_memory>>>(
                    num_parallel_blocks,
                    num_blocks,
                    num_threads,
                    dev_array_neurongroup__spikespace[current_idx_array_neurongroup__spikespace]);

            CUDA_CHECK_ERROR("_run_synapses_4_pre_push_spikes_push_kernel");
        }
    }
