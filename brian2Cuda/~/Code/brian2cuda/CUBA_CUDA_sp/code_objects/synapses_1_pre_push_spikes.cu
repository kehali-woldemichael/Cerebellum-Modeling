    ...
    int32_t* _ptr_array_poissongroup_1__spikespace)
{
    ...
    int32_t spiking_neuron = _ptr_array_poissongroup_1__spikespace[pre_neuron_idx];
...
void _run_synapses_1_pre_push_spikes()
{
    ...
    {
        int num_eventspaces = dev_array_poissongroup_1__spikespace.size();
        synapses_1_pre_eventspace_idx = (current_idx_array_poissongroup_1__spikespace - synapses_1_pre_delay + num_eventspaces) % num_eventspaces;
    ...
    else if (synapses_1_pre_max_size > 0)
    {
        ...
        CUDA_SAFE_CALL(
                cudaMemcpy(&num_spiking_neurons,
                    dev_array_poissongroup_1__spikespace[current_idx_array_poissongroup_1__spikespace] + _num_spikespace - 1,
