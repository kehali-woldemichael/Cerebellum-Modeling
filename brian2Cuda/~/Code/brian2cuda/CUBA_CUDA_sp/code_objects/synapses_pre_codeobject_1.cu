...
__global__ void
kernel_synapses_pre_codeobject_1(
    int _N,
    int bid_offset,
    int timestep,
	...
	double* _ptr_array_synapses_w_MFGR,
	const int _numw_MFGR,
                ...
                int post_block_idx = bid % num_parallel_blocks;
                {
                    ...
                    if(spikes_start <= spiking_neuron && spiking_neuron < spikes_stop)
                    {
                        ...
                        for(int j = tid; j < num_synapses; j+=THREADS_PER_BLOCK)
                        {
                            ...
                            //  Abstract code:  s_AMPA += w_MFGR
                            //  Abstract code:  s_NMDA += w_MFGR
                            const int32_t _postsynaptic_idx = _ptr_array_synapses__synaptic_post[_idx];
                            const double w_MFGR = _ptr_array_synapses_w_MFGR[_idx];
                            _brian_atomicAdd(&_ptr_array_neurongroup_s_AMPA[_postsynaptic_idx], (double)(w_MFGR));
                            _brian_atomicAdd(&_ptr_array_neurongroup_s_NMDA[_postsynaptic_idx], (double)(w_MFGR));
...
void _run_synapses_pre_codeobject_1()
{
    ...
    ///// HOST_CONSTANTS ///////////
    const int _nums_AMPA = 100;
		double* const _array_synapses_w_MFGR = thrust::raw_pointer_cast(&dev_dynamic_array_synapses_w_MFGR[0]);
		const int _numw_MFGR = dev_dynamic_array_synapses_w_MFGR.size();
...
num_loops = 1;
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    kernel_synapses_pre_codeobject_1, num_threads, 0)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, kernel_synapses_pre_codeobject_1)
                );
...
else if (synapses_pre_max_size <= 0)
{
    printf("INFO there are no synapses in the synapses_pre pathway. Skipping synapses_push and synapses kernels.\n");
        ...
        {
            printf("INFO kernel_synapses_pre_codeobject_1\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
if (synapses_pre_max_size > 0)
{
    ...
    if (synapses_pre_scalar_delay)
    {
        if (defaultclock.timestep[0] >= synapses_pre_delay)
        {
            cudaMemcpy(&num_spiking_neurons,
                    &dev_array_poissongroup__spikespace[synapses_pre_eventspace_idx][_num__array_poissongroup__spikespace - 1],
    ...
    if (num_blocks != 0) {
        ...
        {
            kernel_synapses_pre_codeobject_1<<<num_blocks, num_threads>>>(
                _N,
                bid_offset,
                defaultclock.timestep[0],
                num_threads,
                num_threads_per_bundle,
                dev_array_poissongroup__spikespace[synapses_pre_eventspace_idx],
                num_spiking_neurons,
                ///// HOST_PARAMETERS /////
                dev_array_neurongroup_s_AMPA,
			_array_synapses_w_MFGR,
			_numw_MFGR,
    ...
    CUDA_CHECK_ERROR("kernel_synapses_pre_codeobject_1");
}
    ...
    synapses_pre_codeobject_1_profiling_info += _run_time;
}
