...
if (_num__array_statemonitor__indices > 1024)
{
    printf("ERROR in statemonitor: Too many neurons recorded. Due to a bug (brian-team/brian2cuda#50), "
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, kernel_statemonitor_codeobject)
                );
...
{
    kernel_statemonitor_codeobject<<<1, _num__array_statemonitor__indices>>>(
    ...
    CUDA_CHECK_ERROR("kernel_statemonitor_codeobject");
}
