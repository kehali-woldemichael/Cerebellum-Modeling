...
namespace {
    ...
    double host_randn(const int _vectorisation_idx)
    {
        ...
        __global__ void
        _reset_neurongroup_3_thresholder_codeobject(
            int32_t* eventspace
            )
        {
...
__global__ void
kernel_neurongroup_3_thresholder_codeobject(
    int _N,
...
void _run_neurongroup_3_thresholder_codeobject()
{
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    kernel_neurongroup_3_thresholder_codeobject, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    kernel_neurongroup_3_thresholder_codeobject, num_threads, 0)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, kernel_neurongroup_3_thresholder_codeobject)
                );
        ...
        {
            printf("INFO kernel_neurongroup_3_thresholder_codeobject\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
        ...
        _reset_neurongroup_3_thresholder_codeobject<<<num_blocks, num_threads>>>(
                dev_array_neurongroup_3__spikespace[current_idx_array_neurongroup_3__spikespace]
            );

        CUDA_CHECK_ERROR("_reset_neurongroup_3_thresholder_codeobject");

    kernel_neurongroup_3_thresholder_codeobject<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
    ...
    CUDA_CHECK_ERROR("kernel_neurongroup_3_thresholder_codeobject");
