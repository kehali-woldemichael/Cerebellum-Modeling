#include "objects.h"
#include "code_objects/neurongroup_1_thresholder_codeobject_1.h"
...
namespace {
    ...
    double host_randn(const int _vectorisation_idx)
    {
        ...
        __global__ void
        _reset_neurongroup_1_thresholder_codeobject_1(
            int32_t* eventspace
            )
        {
...
__global__ void
kernel_neurongroup_1_thresholder_codeobject_1(
    int _N,
    ...
    )
{
    ...
    const char _cond = V > (-0.052000000000000005);
...
void _run_neurongroup_1_thresholder_codeobject_1()
{
    using namespace brian;
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    kernel_neurongroup_1_thresholder_codeobject_1, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    kernel_neurongroup_1_thresholder_codeobject_1, num_threads, 0)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, kernel_neurongroup_1_thresholder_codeobject_1)
                );
        ...
        {
            printf("INFO kernel_neurongroup_1_thresholder_codeobject_1\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
        ...
        _reset_neurongroup_1_thresholder_codeobject_1<<<num_blocks, num_threads>>>(
                dev_array_neurongroup_1__spikespace[current_idx_array_neurongroup_1__spikespace]
            );

        CUDA_CHECK_ERROR("_reset_neurongroup_1_thresholder_codeobject_1");

    kernel_neurongroup_1_thresholder_codeobject_1<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
    ...
    CUDA_CHECK_ERROR("kernel_neurongroup_1_thresholder_codeobject_1");
    ...
    neurongroup_1_thresholder_codeobject_1_profiling_info += _run_time;
}
