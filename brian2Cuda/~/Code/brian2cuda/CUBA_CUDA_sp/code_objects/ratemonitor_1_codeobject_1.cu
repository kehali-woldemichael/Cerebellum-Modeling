...
__global__ void
kernel_ratemonitor_1_codeobject_1(
	...
	double* _ptr_array_ratemonitor_1_rate,
	const int _numrate,
	const double _value_array_defaultclock_dt,
	double* _ptr_array_ratemonitor_1_t,
	const int _numt
    )
{
    ...
    if (_num_spikespace-1 != 10)  // we have a subgroup
    {
        ...
        {
            const int spiking_neuron = _ptr_array_neurongroup_2__spikespace[i];
            if (spiking_neuron != -1)
            {
    ...
    {
        num_spikes = _ptr_array_neurongroup_2__spikespace[10];
    }

    // TODO: we should be able to use _ptr_array_ratemonitor_1_rate and _ptr_array_ratemonitor_1_t here instead of passing these
    //       additional pointers. But this results in thrust::system_error illegal memory access.
    //       Don't know why... _ptr_array_ratemonitor_1_rate and ratemonitor_rate should be the same...
...
void _run_ratemonitor_1_codeobject_1()
{
    ...
    ///// HOST_CONSTANTS ///////////
    const int _num_spikespace = 11;
		double* const _array_ratemonitor_1_rate = thrust::raw_pointer_cast(&dev_dynamic_array_ratemonitor_1_rate[0]);
		const int _numrate = dev_dynamic_array_ratemonitor_1_rate.size();
		double* const _array_ratemonitor_1_t = thrust::raw_pointer_cast(&dev_dynamic_array_ratemonitor_1_t[0]);
		const int _numt = dev_dynamic_array_ratemonitor_1_t.size();
...
int size_till_now = dev_dynamic_array_ratemonitor_1_t.size();
THRUST_CHECK_ERROR(
        dev_dynamic_array_ratemonitor_1_t.resize(num_iterations + size_till_now - start_offset)
        );
THRUST_CHECK_ERROR(
        dev_dynamic_array_ratemonitor_1_rate.resize(num_iterations + size_till_now - start_offset)
        );
num_threads = 1;
num_blocks = 1;
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    kernel_ratemonitor_1_codeobject_1, num_threads, 0)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, kernel_ratemonitor_1_codeobject_1)
                );
        ...
        {
            printf("INFO kernel_ratemonitor_1_codeobject_1\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
kernel_ratemonitor_1_codeobject_1<<<num_blocks, num_threads>>>(
    current_iteration - start_offset,
    thrust::raw_pointer_cast(&(dev_dynamic_array_ratemonitor_1_rate[0])),
    thrust::raw_pointer_cast(&(dev_dynamic_array_ratemonitor_1_t[0])),
    ///// HOST_PARAMETERS /////
    _array_defaultclock_t[0],
			dev_array_neurongroup_2__spikespace[current_idx_array_neurongroup_2__spikespace],
			_array_ratemonitor_1_rate,
			_numrate,
			_array_defaultclock_dt[0],
			_array_ratemonitor_1_t,
			_numt);

CUDA_CHECK_ERROR("kernel_ratemonitor_1_codeobject_1");
    ...
    ratemonitor_1_codeobject_1_profiling_info += _run_time;
}
