...
__global__ void
kernel_statemonitor_1_codeobject_1(
    int _num_indices,
    int32_t* indices,
...
void _run_statemonitor_1_codeobject_1()
{
    using namespace brian;
...
{
    _dynamic_array_statemonitor_1_V[i].resize(_numt + num_iterations - current_iteration);
...
if (_num__array_statemonitor_1__indices > 1024)
{
    printf("ERROR in statemonitor_1: Too many neurons recorded. Due to a bug (brian-team/brian2cuda#50), "
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, kernel_statemonitor_1_codeobject_1)
                );
...
if(current_iteration >= num_iterations)
{
    for(int i = 0; i < _num__array_statemonitor_1__indices; i++)
    {
        _dynamic_array_statemonitor_1_V[i].resize(_numt + 1);
...
{
    kernel_statemonitor_1_codeobject_1<<<1, _num__array_statemonitor_1__indices>>>(
    ...
    CUDA_CHECK_ERROR("kernel_statemonitor_1_codeobject_1");
}
    ...
    statemonitor_1_codeobject_1_profiling_info += _run_time;
}
