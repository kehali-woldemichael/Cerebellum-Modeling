...
__global__ void
kernel_synapses_5_pre_codeobject(
    int _N,
    int bid_offset,
    int timestep,
...
num_loops = 1;
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    kernel_synapses_5_pre_codeobject, num_threads, 0)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, kernel_synapses_5_pre_codeobject)
                );
...
else if (synapses_5_pre_max_size <= 0)
{
    printf("INFO there are no synapses in the synapses_5_pre pathway. Skipping synapses_push and synapses kernels.\n");
        ...
        {
            printf("INFO kernel_synapses_5_pre_codeobject\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
if (synapses_5_pre_max_size > 0)
{
    ...
    if (num_blocks != 0) {
        ...
        {
            kernel_synapses_5_pre_codeobject<<<num_blocks, num_threads>>>(
                _N,
                bid_offset,
    ...
    CUDA_CHECK_ERROR("kernel_synapses_5_pre_codeobject");
}
