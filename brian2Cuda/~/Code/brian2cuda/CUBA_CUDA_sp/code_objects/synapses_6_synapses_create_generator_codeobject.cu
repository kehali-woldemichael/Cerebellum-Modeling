...
size_t used_device_memory_start = used_device_memory;
    ...
    ///// HOST_CONSTANTS ///////////
    int32_t* const _array_synapses_6_N_incoming = thrust::raw_pointer_cast(&dev_dynamic_array_synapses_6_N_incoming[0]);
		const int _numN_incoming = dev_dynamic_array_synapses_6_N_incoming.size();
		...
		int32_t* const _array_synapses_6_N_outgoing = thrust::raw_pointer_cast(&dev_dynamic_array_synapses_6_N_outgoing[0]);
		const int _numN_outgoing = dev_dynamic_array_synapses_6_N_outgoing.size();
		const int _numN = 1;
    ...
    for(int _i = 0; _i < _N_pre; _i++)
    {
            ...
            for(int _k=_uiter_low; _k<_uiter_high; _k+=_uiter_step)
            {
                ...
                }
                {
                    ...
                    const int32_t i = _i;
                    const char _cond = _brian_abs(i - _k) < 2;
    ...
    const int32_t newsize = _dynamic_array_synapses_6__synaptic_pre.size();
            ...
            THRUST_CHECK_ERROR(
                    dev_dynamic_array_synapses_6_w_BSPKJ.resize(newsize)
                    );
            _dynamic_array_synapses_6_w_BSPKJ.resize(newsize);
