...
void _run_synapses_pre_initialise_queue()
{
    ...
    if (scalar_delay)
    {
        int num_spikespaces = dev_array_poissongroup__spikespace.size();
        if (num_queues > num_spikespaces)
        {
            for (int i = num_spikespaces; i < num_queues; i++)
            {
                ...
                cudaError_t status = cudaMalloc((void**)&new_eventspace,
                        sizeof(int32_t)*_num__array_poissongroup__spikespace);
                if (status != cudaSuccess)
                {
                    printf("ERROR while allocating momory for dev_array_poissongroup__spikespace[%i] on device: %s %s %d\n",
                            i, cudaGetErrorString(status), __FILE__, __LINE__);
                    exit(status);
                }
                dev_array_poissongroup__spikespace.push_back(new_eventspace);
            }
        }
    }
