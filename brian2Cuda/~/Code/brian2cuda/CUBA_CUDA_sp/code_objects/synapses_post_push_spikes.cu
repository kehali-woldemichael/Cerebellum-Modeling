...
__global__ void
_run_synapses_post_push_spikes_push_kernel(
    ...
    int32_t* _ptr_array_neurongroup__spikespace)
{
    ...
    if(synapses_post.spikes_start <= spiking_neuron && spiking_neuron < synapses_post.spikes_stop)
    {
        synapses_post.queue->push_bundles(
            post_neuron_bid,
            tid,
            _num_threads,
            spiking_neuron - synapses_post.spikes_start);
    }
}
...
void _run_synapses_post_push_spikes()
{
    ...
    if (synapses_post_scalar_delay)
    {
        ...
        synapses_post_eventspace_idx = (current_idx_array_neurongroup__spikespace - synapses_post_delay + num_eventspaces) % num_eventspaces;
    ...
    else if (synapses_post_max_size > 0)
    {
        ...
        CUDA_CHECK_ERROR("_run_synapses_post_push_spikes_advance_kernel");
        ...
        if (first_run)
        {
            ...
            num_threads = synapses_post_max_size;
            ...
            CUDA_SAFE_CALL(
                    cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                        _run_synapses_post_push_spikes_push_kernel, num_threads,
            ...
            CUDA_SAFE_CALL(
                    cudaFuncGetAttributes(&funcAttrib, _run_synapses_post_push_spikes_push_kernel)
                    );
            ...
            {
                printf("INFO _run_synapses_post_push_spikes_push_kernel\n"
        ...
        if (num_spiking_neurons > 0)
        {
            ...
            _run_synapses_post_push_spikes_push_kernel<<<num_blocks, num_threads, needed_shared_memory>>>(
                    num_parallel_blocks,
                    num_blocks,
                    num_threads,
                    dev_array_neurongroup__spikespace[current_idx_array_neurongroup__spikespace]);

            CUDA_CHECK_ERROR("_run_synapses_post_push_spikes_push_kernel");
        }
    }
