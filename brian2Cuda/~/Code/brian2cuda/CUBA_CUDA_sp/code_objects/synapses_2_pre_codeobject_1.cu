...
__global__ void
kernel_synapses_2_pre_codeobject_1(
    int _N,
    int bid_offset,
    int timestep,
                ...
                int post_block_idx = bid % num_parallel_blocks;
                {
                    ...
                    if(spikes_start <= spiking_neuron && spiking_neuron < spikes_stop)
                    {
                        ...
                        for(int j = tid; j < num_synapses; j+=THREADS_PER_BLOCK)
                        {
                            ...
                            //  Abstract code:  s_NMDA_1 += 1
                            //  Abstract code:  s_NMDA_2 += 1
                            const int32_t _postsynaptic_idx = _ptr_array_synapses_2__synaptic_post[_idx];
                            _brian_atomicAdd(&_ptr_array_neurongroup_1_s_AMPA[_postsynaptic_idx], (double)(1));
                            _brian_atomicAdd(&_ptr_array_neurongroup_1_s_NMDA_1[_postsynaptic_idx], (double)(1));
                            _brian_atomicAdd(&_ptr_array_neurongroup_1_s_NMDA_2[_postsynaptic_idx], (double)(1));
...
num_loops = 1;
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    kernel_synapses_2_pre_codeobject_1, num_threads, 0)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, kernel_synapses_2_pre_codeobject_1)
                );
...
else if (synapses_2_pre_max_size <= 0)
{
    printf("INFO there are no synapses in the synapses_2_pre pathway. Skipping synapses_push and synapses kernels.\n");
        ...
        {
            printf("INFO kernel_synapses_2_pre_codeobject_1\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
if (synapses_2_pre_max_size > 0)
{
    ...
    if (num_blocks != 0) {
        ...
        {
            kernel_synapses_2_pre_codeobject_1<<<num_blocks, num_threads>>>(
                _N,
                bid_offset,
    ...
    CUDA_CHECK_ERROR("kernel_synapses_2_pre_codeobject_1");
}
    ...
    synapses_2_pre_codeobject_1_profiling_info += _run_time;
}
