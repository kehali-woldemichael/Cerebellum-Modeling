...
__global__ void
kernel_synapses_5_pre_codeobject_1(
    int _N,
    int bid_offset,
    int timestep,
    ...
    )
{
    ...
    {  // _idx is defined in outer and inner scope (for `scalar_code`)
        if (synapses_5_pre.no_or_const_delay_mode)
        {
            ...
            if (timestep >= synapses_5_pre.queue->num_queues - 1)
            {
                ...
                int post_block_idx = bid % num_parallel_blocks;
                {
                    ...
                    if(spikes_start <= spiking_neuron && spiking_neuron < spikes_stop)
                    {
                        ...
                        for(int j = tid; j < num_synapses; j+=THREADS_PER_BLOCK)
                        {
                            ...
                            //  Abstract code:  s_AMPA += 1
                            const int32_t _postsynaptic_idx = _ptr_array_synapses_5__synaptic_post[_idx];
                            _brian_atomicAdd(&_ptr_array_neurongroup_3_s_AMPA[_postsynaptic_idx], (double)(1));
...
num_threads_per_bundle = synapses_5_pre_max_bundle_size;
num_loops = 1;
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    kernel_synapses_5_pre_codeobject_1, num_threads, 0)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, kernel_synapses_5_pre_codeobject_1)
                );
...
else if (synapses_5_pre_max_size <= 0)
{
    printf("INFO there are no synapses in the synapses_5_pre pathway. Skipping synapses_push and synapses kernels.\n");
        ...
        {
            printf("INFO kernel_synapses_5_pre_codeobject_1\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
if (synapses_5_pre_max_size > 0)
{
    ...
    if (synapses_5_pre_scalar_delay)
    {
        if (defaultclock.timestep[0] >= synapses_5_pre_delay)
        {
            cudaMemcpy(&num_spiking_neurons,
                    &dev_array_neurongroup__spikespace[synapses_5_pre_eventspace_idx][_num__array_neurongroup__spikespace - 1],
    ...
    if (num_blocks != 0) {
        ...
        {
            kernel_synapses_5_pre_codeobject_1<<<num_blocks, num_threads>>>(
                _N,
                bid_offset,
                defaultclock.timestep[0],
                num_threads,
                num_threads_per_bundle,
                dev_array_neurongroup__spikespace[synapses_5_pre_eventspace_idx],
                num_spiking_neurons,
                ///// HOST_PARAMETERS /////
    ...
    CUDA_CHECK_ERROR("kernel_synapses_5_pre_codeobject_1");
}
    ...
    synapses_5_pre_codeobject_1_profiling_info += _run_time;
}
