#include "objects.h"
#include "code_objects/synapses_5_post_codeobject_1.h"
...
__global__ void
kernel_synapses_5_post_codeobject_1(
    int _N,
    int bid_offset,
    int timestep,
    ...
    )
{
    ...
    {  // _idx is defined in outer and inner scope (for `scalar_code`)
        if (synapses_5_post.no_or_const_delay_mode)
        {
            // TODO: pass as kernel parameter instead?
            int num_parallel_blocks = synapses_5_post.queue->num_blocks;
            int32_t spikes_start = synapses_5_post.spikes_start;
            int32_t spikes_stop = synapses_5_post.spikes_stop;
            ...
            if (timestep >= synapses_5_post.queue->num_queues - 1)
            {
                ...
                int post_block_idx = bid % num_parallel_blocks;
                {
                    ...
                    if(spikes_start <= spiking_neuron && spiking_neuron < spikes_stop)
                    {
                        ...
                        int num_synapses = synapses_5_post_num_synapses_by_pre[pre_post_block_id];
                        int32_t* propagating_synapses = synapses_5_post_synapse_ids_by_pre[pre_post_block_id];
                        for(int j = tid; j < num_synapses; j+=THREADS_PER_BLOCK)
                        {
                            ...
                            //  Abstract code:  s_AHP_BS = 1
                            const int32_t _postsynaptic_idx = _ptr_array_synapses_5__synaptic_post[_idx];
                            double s_AHP_BS;
                            s_AHP_BS = 1;
                            _ptr_array_neurongroup_3_s_AHP_BS[_postsynaptic_idx] = s_AHP_BS;
        ...
        else  // heterogeneous delay mode
        {
            ...
            synapses_5_post.queue->peek(&synapses_queue);
            ...
            for (int i = tid; i < queue_size*threads_per_bundle; i+=THREADS_PER_BLOCK)
            {
                ...
                int bundle_size = synapses_5_post_num_synapses_by_bundle[bundle_id];
                int synapses_offset = synapses_5_post_synapses_offset_by_bundle[bundle_id];
                int32_t* synapse_ids = synapses_5_post_synapse_ids;
...
void _run_synapses_5_post_codeobject_1()
{
    using namespace brian;
    ...
    ///// HOST_CONSTANTS ///////////
    const int _nums_AHP_BS = 10;
		int32_t* const _array_synapses_5__synaptic_pre = thrust::raw_pointer_cast(&dev_dynamic_array_synapses_5__synaptic_pre[0]);
		const int _num_synaptic_pre = dev_dynamic_array_synapses_5__synaptic_pre.size();
		int32_t* const _array_synapses_5__synaptic_post = thrust::raw_pointer_cast(&dev_dynamic_array_synapses_5__synaptic_post[0]);
...
num_threads_per_bundle = synapses_5_post_max_bundle_size;
num_loops = 1;
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    kernel_synapses_5_post_codeobject_1, num_threads, 0)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, kernel_synapses_5_post_codeobject_1)
                );
...
else if (synapses_5_post_max_size <= 0)
{
    printf("INFO there are no synapses in the synapses_5_post pathway. Skipping synapses_push and synapses kernels.\n");
}
        else
        {
            printf("INFO kernel_synapses_5_post_codeobject_1\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
if (synapses_5_post_max_size > 0)
{
    ...
    if (synapses_5_post_scalar_delay)
    {
        if (defaultclock.timestep[0] >= synapses_5_post_delay)
        {
            cudaMemcpy(&num_spiking_neurons,
                    &dev_array_neurongroup_3__spikespace[synapses_5_post_eventspace_idx][_num__array_neurongroup_3__spikespace - 1],
    ...
    if (num_blocks != 0) {
        ...
        {
            kernel_synapses_5_post_codeobject_1<<<num_blocks, num_threads>>>(
                _N,
                bid_offset,
                defaultclock.timestep[0],
                num_threads,
                num_threads_per_bundle,
                dev_array_neurongroup_3__spikespace[synapses_5_post_eventspace_idx],
                num_spiking_neurons,
                ///// HOST_PARAMETERS /////
                dev_array_neurongroup_3_s_AHP_BS,
			_array_synapses_5__synaptic_pre,
			_num_synaptic_pre,
			_array_synapses_5__synaptic_post,
    ...
    CUDA_CHECK_ERROR("kernel_synapses_5_post_codeobject_1");
}
    ...
    synapses_5_post_codeobject_1_profiling_info += _run_time;
}

void _debugmsg_synapses_5_post_codeobject_1()
{
    using namespace brian;
