...
void Network::run(const double duration, void (*report_func)(const double, const double, const double, const double), const double report_period)
{
    ...
    while(clock && clock->running())
    {
        ...
        brian::current_idx_array_poissongroup_1__spikespace = (brian::current_idx_array_poissongroup_1__spikespace + 1) % brian::dev_array_poissongroup_1__spikespace.size();
        brian::current_idx_array_poissongroup__spikespace = (brian::current_idx_array_poissongroup__spikespace + 1) % brian::dev_array_poissongroup__spikespace.size();
